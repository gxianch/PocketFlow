...
本项目演示了思维链（Chain-of-Thought）流程的实现编排，使 LLM 能够通过逐步思考来解决复杂推理问题。它旨在通过外部管理的深思熟虑、结构化推理来提高问题解决的准确性。

此实现基于教程：[Build Chain-of-Thought From Scratch - Tutorial for Dummies](https://zacharyhuang.substack.com/p/build-chain-of-thought-from-scratch)。

## 功能特性

- 提升模型在复杂问题上的推理能力
- 利用强大的指令跟随模型（如 Claude 3.7 Sonnet、GPT-4 系列）执行结构化思维链推理
- 通过系统分解解决直接提示经常失败的问题
- 提供详细的推理跟踪，包括逐步评估和规划，便于验证
...
4. **运行默认示例：**
    执行主脚本以使用默认的 Jane Street 问题查看流程。
    ```bash
    python main.py
    ```
    默认问题是：
    > 你不断掷一个公平的骰子，直到连续三次掷出 3、4、5 的顺序。你掷骰子的次数为奇数的概率是多少？
...
实现使用自循环 PocketFlow 节点（`ChainOfThoughtNode`）引导 LLM 经历结构化问题解决流程：
...
在每次循环（思考步骤）中，节点引导 LLM：
1. 评估前一次思考的推理和结果
2. 根据维护的计划执行下一个待定步骤
3. 更新计划，标记步骤完成（带结果）或记录问题
4. 如果需要分解步骤或纠正错误，则优化计划
5. 根据计划状态决定是否需要进一步思考（`next_thought_needed`）

这种外部编排强制采用系统性方法，帮助模型处理单个提示难以解决的问题。

## 与不同方法的比较

- **标准提示**：在单个提示中要求模型"逐步思考"等技术可能有所帮助，但推理可能缺乏深度或结构，模型很容易失去跟踪或犯下无法恢复的错误。
- **原生扩展思考模式**：某些模型（如 Claude 3.7、GPT-o1 等）提供专门用于扩展推理的专用模式或功能，通常通过 API 调用直接产生强大结果。
- **此实现**：演示如何使用标准 LLM（即使那些没有特定原生'扩展思考'模式的 LLM）编排结构化思维链流程，通过提示工程和流程控制外部管理步骤、规划和评估。

## 示例思考过程

让我们尝试这个具有挑战性的 [Jane Street 量化交易面试问题](https://www.youtube.com/watch?v=gQJTkuEVPrU)：

> **问题**：你不断掷一个公平的骰子，直到连续三次掷出 3、4、5 的顺序。你掷骰子的次数为奇数的概率是多少？

此问题演示了结构化思维链的价值：

- **标准模型（单个提示）**：经常给出错误答案或提供有缺陷的推理。
- **使用原生思考模式的模型**：能找到正确答案（216/431 ≈ 0.5012），尽管性能和推理清晰度可能有所不同。
- **此实现（编排有能力的 LLM）**：通过强制逐步计划、评估和优化循环，能够引导模型朝向正确答案。

作为比较：
- [Claude 3.7 Sonnet（单个提示）](https://claude.ai/share/da139326-42fe-42d9-9d7b-35870daa5c1b)：错误答案
- [Claude 3.7 Sonnet（使用内置思考）](https://claude.ai/share/6f4140ed-f33c-4949-8778-a57719498e40)：3 分 45 秒后得出正确答案
- [GPT-o1（使用内置思考）](https://chatgpt.com/share/67fee0fd-2600-8000-bcdf-76e40a986ee4)：2 分 0 秒后得出正确答案
- [GPT-o1 pro（使用内置思考）](https://chatgpt.com/share/67fee11b-530c-8000-92d1-609b6ca49c9c)：4 分 24 秒后得出正确答案

下面是一个示例输出跟踪，显示**此实现如何指导 Claude 3.7 Sonnet** 解决问题解决流程：

> **注意**：即使有结构化思考编排，模型也不总是能得到正确答案，特别是在非常复杂或新颖的问题上。然而，这种方法显著提高了推理过程的稳健性，并提供了可追踪的路径以便验证和调试。
...
Thought 3:
  ...
  G₁/6·G(z) - z·G₂(z₁(z) = 4z/6·G₀(z) + z/6·G₂(z)
