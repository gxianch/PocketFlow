# OpenAI 嵌入向量与 PocketFlow

此示例演示如何正确地将 OpenAI 的文本嵌入 API 与 PocketFlow 集成，重点关注：

1. 关注点分离的清洁代码组织：
   - API 交互的工具层（`tools/embeddings.py`）
...
├── tools/
│   └── embeddings.py     # OpenAI 嵌入 API 封装器
├── utils/
...
这将：
1. 从环境加载 OpenAI API 密钥
2. 创建 PocketFlow 节点来处理嵌入生成
3. 处理示例文本并生成其嵌入
4. 显示嵌入维度和前几个值
