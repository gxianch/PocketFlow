...
使用 PocketFlow、FastAPI 和 WebSocket 实现流式 LLM 响应的实时聊天界面。
...
- **实时流式传输**：查看 AI 响应在 LLM 生成过程中的实时显示
- **对话记忆**：跨消息维护聊天历史
- **现代 UI**：清洁、响应式的聊天界面，采用渐变设计
- **WebSocket 连接**：持久连接，实现即时通信
- **PocketFlow 集成**：使用 PocketFlow 的 `AsyncNode` 和 `AsyncFlow` 进行流式传输
...
- [`utils/stream_llm.py`](./utils/stream_llm.py)：OpenAI 流式传输工具
