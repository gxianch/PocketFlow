...
演示如何使用 Map-Reduce 模式处理和评估简历的 PocketFlow 示例。
...
工作流程遵循经典的 Map-Reduce 模式，包含三个顺序节点：
...
1. **ReadResumesNode（Map 阶段）**：从数据目录读取所有简历文件并将它们存储在共享数据存储中
2. **EvaluateResumesNode（批处理）**：使用 LLM 单独处理每份简历，以确定候选人是否符合要求
3. **ReduceResultsNode（Reduce 阶段）**：聚合评估结果并生成合格候选人摘要

## 文件说明

- [`main.py`](./main.py)：运行简历筛选工作流程的主要入口点
- [`flow.py`](./flow.py)：定义连接节点的流程
- [`nodes.py`](./nodes.py)：包含工作流程中每个步骤的节点类
- [`utils.py`](./utils.py)：实用函数，包括 LLM 封装器
